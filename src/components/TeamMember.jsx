...
export default function TeamMember() {
  ...
  useEffect(() => {
    axios
      .get("https://meds-scan-backend.onrender.com/api/team", {
  ...
  const handleAddMember = (e) => {
    e.preventDefault();
    axios
      .post("https://meds-scan-backend.onrender.com/api/team", newMember, {
        headers: { Authorization: `Bearer ${getToken()}` },
      })
      .then((response) => {
  ...
  const handleDeleteMember = (id) => {
    axios
      .delete(`https://meds-scan-backend.onrender.com/api/team/${id}`, {
        headers: { Authorization: `Bearer ${getToken()}` },
      })
      .then(() => {
      ...
      {teamMembers.length > 0 ? (
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8 max-sm:grid-cols-1 sm:grid-cols-1">
          {teamMembers.map((member) => (
            <div
              key={member._id}
              className="text-center bg-white shadow-lg rounded-md relative p-5 max-sm:w-[100%]"
            >
                ...
                {dropdownOpen === member._id && (
                  <div className="absolute bottom-14 right-0 mt-2 bg-white border rounded-md shadow-lg">
                    ...
                    <div className="flex items-center gap-1 px-4 py-2 text-green-500 hover:bg-gray-100 w-full">
                      <MdMessage />
                      <button onClick={() => handleMessageClick(member._id)}>
                        Message
                      </button>
                    </div>
                  </div>
