...
import { ComposedChart, PieChart, Pie, Bar, BarChart, XAxis, YAxis, Tooltip, Legend, CartesianGrid } from 'recharts';
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        const formattedSalesTrend = salesTrendRes.data.salesTrend.map(item => ({
          date: item.day ? new Date(item.day).toLocaleDateString() : 'Invalid Date',
          totalSales: item.sales || 0,
        }));

        // Format Market Share Data
        const formattedMarketShare = Object.keys(marketShareRes.data.marketShare).map(key => ({
          name: key,
          value: marketShareRes.data.marketShare[key] || 0,
        }));

        // Format Top Products Data
        const formattedTopProducts = topProductsRes.data.topProducts.map(item => ({
    ...
    <div className="flex">
      <aside className="w-64 bg-gray-800 text-white h-screen p-6">
        <nav className="space-y-4">
          ...
          <Link to="/allProducts" className="block py-2.5 px-4 rounded transition duration-200 hover:bg-gray-700">All Products</Link>
          <Link to="/addNewPro" className="block py-2.5 px-4 rounded transition duration-200 hover:bg-gray-700">Add New Product</Link>
          ...
          <Link to="/team" className="block py-2.5 px-4 rounded transition duration-200 hover:bg-gray-700">Team Members</Link>
          <Link to="/support" className="block py-2.5 px-4 rounded transition duration-200 hover:bg-gray-700">Support</Link>
          <Link to="/addStock" className="block py-2.5 px-4 rounded transition duration-200 hover:bg-gray-700">Add Stock</Link>
        </nav>
      </aside>
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-white p-6 rounded-lg shadow">
            ...
            <PieChart width={300} height={300}>
              <Pie data={marketShare} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={100} fill="#8884d8" label />
              <Tooltip />
            </PieChart>
          </div>
